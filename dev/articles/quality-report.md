...
The variance card documents the design of the replication (including the
degrees of freedom and whether a finite-population correction was
applied), and it flags when a calibration targeted estimated totals from
a
[`reference_sample()`](https://jpferreira33.github.io/weightflow/dev/reference/reference_sample.md).
The standard errors, CVs and confidence intervals of specific estimates
are computed downstream with the `survey` or `srvyr` package (see the
Variance estimation article).

The HTML report is the human-facing view. For the same information as a
programmatic quality-control surface (alerts as a gate, per-unit and
per-domain inspection), see
