...
To make every unit comparable one-to-one, the recipes below use **only**
the calibration step (no dropping or nonresponse), so no rows are
removed.
...
Post-stratifying to the population counts of `region`: each region’s
...
Raking (iterative proportional fitting) to the `region` and `sex`
margins. We tighten `survey`’s convergence so both solve the system to
the same precision.
...
The agreement carries over to estimates. A calibrated total of a survey
