...
#' Example nonprobability sample
#'
#' A small nonprobability (convenience) sample whose participation depends on
#' covariates correlated with the outcome, so the naive mean of `income` is
#' biased upward relative to the population. It has no design weights, since the
#' inclusion probabilities are unknown by construction. Use it with
#' `reference_sample` to derive pseudo-weights via `step_pseudo_weight()`.
#' Generated by `data-raw/nonprob_data.R`.
#'
#' @format A data frame with one row per unit:
#' \describe{
#'   \item{person_id}{identifier (from the synthetic population)}
#'   \item{region, sex, age}{common covariates, shared with the reference}
#'   \item{income}{the study outcome}
#' }
#' @seealso \code{\link{reference_sample}}, \code{\link{step_pseudo_weight}}
"nps_sample"

#' Example probability reference sample
#'
#' A large probability reference sample drawn from the same synthetic population
#' as `nps_sample`, with design weights `pw_ref` (the inverse inclusion
#' probabilities) that sum to the population size. The weighted reference
#' represents the population, so it can anchor the participation-propensity model
#' used to build pseudo-weights for the nonprobability sample. Generated by
#' `data-raw/nonprob_data.R`.
#'
#' @format A data frame with one row per sampled unit:
#' \describe{
#'   \item{person_id}{identifier (from the synthetic population)}
#'   \item{region, sex, age}{common covariates, shared with the nonprobability sample}
#'   \item{income}{the study outcome}
#'   \item{pw_ref}{design weight (inverse inclusion probability)}
#' }
#' @seealso \code{\link{nps_sample}}, \code{\link{step_pseudo_weight}}
"reference_sample"
