...
* `boot_mean()` / `jack_mean()` and `collect_replicate_weights()` now treat active negative weights consistently with the totals estimators and `as_svydesign()`/`as_svrepdesign()` (they are kept, not dropped), so the two export routes describe the same sample. **Behaviour change:** estimates and standard errors may shift where a linear/GREG calibration produced negative weights (previously those units were silently excluded from the means and the srvyr export); the new results are the correct, consistent ones.
...
* Trimming redistributes the trimmed mass by an equal split when the free set's weight sum is not clearly positive (negative weights from an earlier GREG), instead of the proportional formula that could explode or flip sign.
* `domain_summary()` no longer errors when the domain column already contains a literal `(missing)` category alongside real `NA`s (a distinct label is used for the NAs).
* `step_calibrate(method = "linear")` rejects a single data frame as `totals` at construction, pointing to the named-vector or tidy-list format, instead of failing later with an opaque "names must match the model.matrix columns" error.
* Report fixes: the closing line only claims "calibration constraints preserved" when the recipe has a constraint step and only reports replicate weights for a real bootstrap/jackknife object; the deff amber threshold in the tables matches the closing interpretation (>= 1.4); the chi-square distance keeps its decimals; a non-finite deff or difference shows an em dash instead of a green "NA"/"+0.000"; the step count reads "1 step" in the singular (English and Spanish, including verb agreement in the Spanish summary).
